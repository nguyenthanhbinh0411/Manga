...
async function fetchSearchSuggestions(keyword) {
    if (!keyword.trim()) {
        suggestionsBox.style.display = 'none'; // Ẩn nếu không có từ khóa
        return;
    }
    ...
    try {
        ...
        // Xóa nội dung cũ của hộp gợi ý
        suggestionsBox.innerHTML = '';

        if (comics && comics.length > 0) {
            const chapterFetches = comics.map(async (comic) => {
                // Tạo gợi ý ban đầu với dữ liệu tối thiểu
                let latestChapterHtml = '<p>Đang cập nhật...</p>';
                let updatedDateHtml = `<p style="font-size: 12px;"><i>Cập nhật: ${formatDate(comic.updatedAt)}</i></p>`;

                // Tạo phần tử gợi ý
                const suggestionItem = document.createElement('div');
                suggestionItem.classList.add('suggestion-item');
                suggestionItem.innerHTML = `
                    <img src="https://img.otruyenapi.com/uploads/comics/${comic.slug}-thumb.jpg" alt="${comic.name}" loading="lazy" onerror="this.src='placeholder.jpg';">
                    <div>
                        <h4>${comic.name}</h4>
                        <span>${latestChapterHtml}</span>
                        ${updatedDateHtml}
                    </div>
                `;
                suggestionsBox.appendChild(suggestionItem);

                // Thay đổi nội dung của phần tử gợi ý sau khi tải chương mới nhất
                try {
                    ...
                    if (Array.isArray(latestChapters) && latestChapters.length > 0) {
                        ...
                        const chapterApiData = latestChapter.link || latestChapter.api_data || latestChapter.chapter_api_data;
                        if (chapterApiData) {
                            latestChapterHtml = `
                                <a href="MangaDetails.html?slug=${comic.slug}&chapter=${encodeURIComponent(chapterApiData)}">
                                    Chương ${latestChapter.chapter_name}
                                </a>
                            `;
                        }
                    } else {
                        latestChapterHtml = `<p>Chưa có chương mới.</p>`;
                    }
                } catch (error) {
                    console.error(`Lỗi khi lấy chương mới nhất cho ${comic.slug}:`, error);
                    latestChapterHtml = `<p>Lỗi khi tải chương mới.</p>`;
                }

                // Cập nhật nội dung sau khi có dữ liệu chương mới
                suggestionItem.querySelector('span').innerHTML = latestChapterHtml;

                // Thêm sự kiện click để chuyển đến trang chi tiết truyện
            ...
            // Chờ tất cả yêu cầu fetch chương mới nhất hoàn tất
            await Promise.allSettled(chapterFetches);

            suggestionsBox.style.display = 'block'; // Hiển thị hộp gợi ý
        } else {
            suggestionsBox.innerHTML = '<p>Không tìm thấy truyện nào</p>';
            suggestionsBox.style.display = 'block';
        }
    } catch (error) {
        console.error('Lỗi khi tìm kiếm:', error);
        suggestionsBox.innerHTML = '<p>Đã xảy ra lỗi trong quá trình tìm kiếm</p>';
        suggestionsBox.style.display = 'block';
    }
}

// Thêm debounce vào sự kiện input để giảm số lần gọi API
